...
public class AuthController {
    ...
    public ResponseEntity<?> verifyOtp(@Valid @RequestBody com.example.cafe_manager_api.dto.VerifyOtpRequest request) {
        try {
            LoginResponse response = authService.verifyOtp(request);
            return ResponseEntity.ok(response);
        } catch (BadCredentialsException ex) {
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body(Map.of("code", "INVALID_OTP", "message", ex.getMessage()));
        }
    }

    @PostMapping("/resend-otp")
    public ResponseEntity<?> resendOtp(@Valid @RequestBody com.example.cafe_manager_api.dto.ResendOtpRequest request) {
        try {
            authService.resendOtp(request);
            return ResponseEntity.ok(Map.of("message", "Mã xác thực đã được gửi lại thành công."));
        } catch (BadCredentialsException ex) {
