...
public class ShiftReportService {
    ...
    public DailyShiftReportResponse getDailyShiftReport(String dateStr) {
        ...
        List<ShiftEntity> rawShifts = shiftRepository.filterShifts(shiftDate, null);
        List<ShiftEntity> shifts = rawShifts.stream()
                .filter(s -> s.getStatus() != null && !s.getStatus().equalsIgnoreCase("DRAFT") && !s.getStatus().equalsIgnoreCase("CANCELLED"))
                .collect(Collectors.toList());
