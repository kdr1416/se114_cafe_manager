...
public class OrdersListAdapter extends RecyclerView.Adapter<OrdersListAdapter.OrderVH> {
    ...
    static class OrderVH extends RecyclerView.ViewHolder {
        ...
        void bind(OrderWithItems order, OnPaymentClickListener listener) {
            tvTable.setText("Bàn " + order.getOrder().getTableId());
            tvCode.setText("Order #" + order.getOrder().getOrderId());
            tvItems.setText(order.getItems().size() + " món");
            tvTotal.setText(CurrencyUtils.formatVnd(order.getOrder().getTotalAmount()));
